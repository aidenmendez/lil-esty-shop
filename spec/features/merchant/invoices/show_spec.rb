...
RSpec.describe 'invoices show' do
  before :each do
    ...
    # for testing discounts from multiple merchants
    @invoice_9 = Invoice.create!(merchant_id: @merchant2.id, customer_id: @customer_2.id, status: 1)    
    ...
    # for testing discounts from multiple merchants
    @ii_12 = InvoiceItem.create!(invoice_id: @invoice_9.id, item_id: @item_5.id, quantity: 20, unit_price: 30, status: 1)
    @ii_13 = InvoiceItem.create!(invoice_id: @invoice_9.id, item_id: @item_6.id, quantity: 20, unit_price: 50, status: 1)
    @ii_14 = InvoiceItem.create!(invoice_id: @invoice_9.id, item_id: @item_3.id, quantity: 12, unit_price: 100, status: 1)
  ...
  it "shows the total discounted amount" do
    ...
    expect(page).to have_content("Discounts Applied: #{@invoice_1.total_discounted}")
  end
  ...
  it "can discount items from separate merchants on the same invoice" do
